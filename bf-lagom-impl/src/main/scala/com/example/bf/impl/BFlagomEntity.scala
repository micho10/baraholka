...
/**
  * This is an event sourced entity. It has a state, [[BFlagomState]], which
...
class BFlagomEntity extends PersistentEntity {
  ...
  override type Command = BFlagomCommand[_]
  override type Event = BFlagomEvent
  override type State = BFlagomState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: BFlagomState = BFlagomState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case BFlagomState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        BFlagomState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class BFlagomState(message: String, timestamp: String)
...
object BFlagomState {
  /**
    ...
    */
  implicit val format: Format[BFlagomState] = Json.format
}

/**
  * This interface defines all the events that the HellolagomEntity supports.
  */
sealed trait BFlagomEvent extends AggregateEvent[BFlagomEvent] {
  def aggregateTag = BFlagomEvent.Tag
}

object BFlagomEvent {
  val Tag = AggregateEventTag[BFlagomEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends BFlagomEvent
...
/**
  ...
  */
sealed trait BFlagomCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends BFlagomCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends BFlagomCommand[String]
